...
def run_comparison(
    ...
    **kwargs
    ...
    df_ne = df_cmp.filter(
        pl.col("creturn").round(6) != pl.col("creturn_long").round(6)
    )
    ...
    assert df_ne.is_empty(), f"Found differences in creturn"
...
def run_trades_comparison(
    ...
    **kwargs
    ...
    if wide_trade_count > 0:
        ...
        assert diff_ratio < 0.1, f"Trade count differs by >10%: wide={wide_trade_count}, long={long_trade_count}"
...
@pytest.mark.parametrize("fee_ratio,tax_ratio", [
    (0, 0),
    (0.001425, 0.003),
    (0.01, 0.005),
])
...
def test_polars_rolling_null(long_format_df):
    ...
    # Add weight with polars rolling (produces nulls)
    df_with_weight = add_weight_to_long(df_long)

    # Check that nulls exist
    null_count = df_with_weight.select(pl.col("weight").null_count()).item()
