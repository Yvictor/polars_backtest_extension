...
// ============================================================================
// Long Format vs Wide Format Consistency Tests
// ============================================================================

use btcore::simulation::{backtest_long, LongFormatInput};

/// Helper: Convert wide format to long format
fn wide_to_long(
    prices: &[Vec<f64>],
    weights: &[Vec<f64>],
    rebalance_indices: &[usize],
) -> (Vec<u32>, Vec<u32>, Vec<f64>, Vec<f64>, Vec<bool>) {
    let n_dates = prices.len();
    let n_symbols = if prices.is_empty() { 0 } else { prices[0].len() };

    let mut date_indices = Vec::new();
    let mut symbol_ids = Vec::new();
    let mut long_prices = Vec::new();
    let mut long_weights = Vec::new();

    // Build weight lookup
    let mut weight_idx = 0;
    let mut current_weights = vec![0.0; n_symbols];

    for (t, price_row) in prices.iter().enumerate() {
        // Update weights if this is a rebalance day
        if rebalance_indices.contains(&t) && weight_idx < weights.len() {
            current_weights = weights[weight_idx].clone();
            weight_idx += 1;
        }

        for (s, &price) in price_row.iter().enumerate() {
            date_indices.push(t as u32);
            symbol_ids.push(s as u32);
            long_prices.push(price);
            long_weights.push(if s < current_weights.len() {
                current_weights[s]
            } else {
                0.0
            });
    ...
    let rebalance_mask: Vec<bool> = (0..n_dates)
        .map(|t| rebalance_indices.contains(&t))
        .collect();

    (date_indices, symbol_ids, long_prices, long_weights, rebalance_mask)
}

#[test]
fn test_long_vs_wide_basic() {
    // Simple case: 4 days, 2 stocks, equal weight
    let prices = vec![
        vec![100.0, 100.0],
        vec![100.0, 100.0],
        vec![110.0, 90.0],
    ...
    // Wide format
    let wide_result = run_backtest(&prices, &signals, &rebalance_indices, &config);

    // Convert to long format
    let (date_indices, symbol_ids, long_prices, long_weights, rebalance_mask) =
        wide_to_long(&prices, &signals, &rebalance_indices);
    ...
    let long_result = backtest_long(&input, prices.len(), 2, &rebalance_mask, &config);

    // Compare results
    assert_eq!(wide_result.len(), long_result.creturn.len());
    for (i, (w, l)) in wide_result.iter().zip(long_result.creturn.iter()).enumerate() {
        assert!(
            (w - l).abs() < 1e-10,
            "Day {}: wide={}, long={}, diff={}",
            i, w, l, (w - l).abs()
...
#[test]
fn test_long_vs_wide_with_fees() {
    let prices = vec![
        vec![100.0],
        vec![100.0],
        vec![110.0],
    ...
    let long_result = backtest_long(&input, prices.len(), 1, &rebalance_mask, &config);
...
#[test]
fn test_long_vs_wide_multiple_rebalances() {
    let prices = vec![
        vec![100.0, 100.0],
        vec![100.0, 100.0],
        vec![110.0, 90.0],
    ...
    // Switch from stock 0 to stock 1 on day 3
    let signals = vec![
        vec![1.0, 0.0],  // Day 0: stock 0 only
        vec![0.0, 1.0],  // Day 3: stock 1 only
    ];
    let rebalance_indices = vec![0, 3];
...
#[test]
fn test_long_vs_wide_stop_loss() {
    let prices = vec![
        vec![100.0],
        vec![100.0],
        vec![95.0],
    ...
    println!("wide: {:?}", wide_result);
    println!("long: {:?}", long_result.creturn);
